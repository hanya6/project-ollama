...
Penggunaan:
    ...
    python evaluate.py --input results/hasil_klasifikasi.csv  # File hasil tertentu
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluasi Hasil Klasifikasi Tema Laporan Posyandu"
    )
...
def main():
    ...
    print("""
╔══════════════════════════════════════════════════════════════╗
║  EVALUASI KLASIFIKASI TEMA LAPORAN POSYANDU                 ║
║  Zero-Shot Learning dengan Large Language Model             ║
╚══════════════════════════════════════════════════════════════╝
    """)
    ...
    # Cek apakah ada kolom label aktual
    if "tema_aktual" not in df.columns:
        ...
        print("[INFO] Pastikan file CSV memiliki kolom 'tema_aktual' dan 'tema_prediksi'.")
        sys.exit(1)
    ...
    print(f"[INFO] Label aktual unik: {len(set(y_true))}")
    print(f"[INFO] Label prediksi unik: {len(set(y_pred))}")
